...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = $('.primary_structure > li:gt(0) > a').map((i, a) => $(a).attr('href')?.trim()).get()
...
async function findAllPagesLinks(page, mainLinks) {
    ...
    for (let i = 0; i < mainLinks.length; i++) {
        try {
            ...
            const paginationElement = $('.MuiPagination-ul');
            console.log("Pagination Element : ", paginationElement.length);
            if (paginationElement.length) {

                let lsatPageNumber = $('.MuiPagination-ul > li')?.last().text()?.trim();
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $('.product-item-title-wrapper')
                    .map((i, e) => $(e).attr('href'))
                    .get()
                ...
                nextPageBtn = await page.$$('.paginationjs-next')
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ['https://kashiland.com/store/']
